...
const RideDetailsPanel: React.FC<RideDetailsPanelProps> = ({ currentRide }) => {
  ...
  return (
    <motion.div 
      className="absolute top-16 left-0 right-0 bg-white rounded-b-3xl shadow-lg border-b border-rideroot-mediumGrey/30 z-20"
      animate={{ 
        height: expanded ? "auto" : "4rem",
      ...
      <AnimatePresence>
        {expanded && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <ScrollArea className="px-4 py-3 max-h-[70vh]">
              {currentRide.driver && (
                <motion.div 
